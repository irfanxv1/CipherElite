...
# CipherElite Custom Imports & Event Handlers
...
# -------------------------------------------------------------------
# CIPHERELITE AUTOMATOR ENGINE
# -------------------------------------------------------------------
@userbot.on(userbot.events.NewMessage(chats=TARGET_BOTS))
@userbot.on(userbot.events.MessageEdited(chats=TARGET_BOTS))
...
# TEST COMMAND TO CHECK IF PLUGIN IS ALIVE
@userbot.on(userbot.events.NewMessage(pattern=r"\.autot", outgoing=True))
async def autot_test(event):
    await edit_or_reply(event, "✅ **Auto Task Automator (autot) Module Active!**")
